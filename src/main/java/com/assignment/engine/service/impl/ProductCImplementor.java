 ...
 *
 */
public class ProductCImplementor implements PromotionImplementor {

	/**
	 * 
	 */
	public ProductCImplementor() {
					...
					for (int z = 0 ; z < prom.size() ; z++)
					{
						if(id == prom.get(z).getProdid())
						{
			    	    	...
			    	    	//	appliedprice += ((m*prom.get(z).getDisctprice())+(y*prods.get(i).getValue()));
							   ...
							   {
								  Map<Character, Integer> check = null;
								  check = checkDependency(prom.get(z),purchases);
								 if(null != check)
								 {
	...
	//	System.out.println("size of mapped is"+mapped.size());
		return mapped;
	}
}
